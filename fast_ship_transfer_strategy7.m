function [id,t_arrival,delv1,delv2]=fast_ship_transfer_strategy7(t_departure,r_search_min,r_search_max,theta_min,theta_max,x,y,z,vx,vy,vz,i_vec,star_ID)
...
star_positions_target=[x(star_ID+1,i_arrival),y(star_ID+1,i_arrival),z(star_ID+1,i_arrival)]; % Except sun, all position values for stars at t=tof


% Filter stars based on position
r_stars=(vecnorm(star_positions_target'))';
idx_fs_targets= find((r_stars>r_search_min) & (r_stars<r_search_max));
...
for j=1:length(idx_fs_targets)
    ...
    while(1)
        ...
        if (delv_transfer * kpcpmyr2kms + delv_rendezvous * kpcpmyr2kms) > 1500 || r_min < 2 % If Fast ship total impulse exceeds  1500 km/s
            ...
            store=[store; idx tof (delv_transfer * kpcpmyr2kms + delv_rendezvous * kpcpmyr2kms) r_stars(idx_fs_targets(j))  theta_star delv1*kpcpmyr2kms delv2*kpcpmyr2kms t_arrival idx_fs_targets(j)];
            break
        end
...
if ~isempty(store)
    ...
    row_id=store(1,end);              % row starID
    id=star_ID(row_id);
    t_arrival=store(1,end-1);     % myr
    delv1 = store(1,6:8);       % kpc/myr
    delv2 = store(1,9:11);      % kpc/myr
